...
[[img-russel]]
.http://commons.wikimedia.org/wiki/File:Russell_in_1924_01.jpg#mediaviewer/File:Russell_in_1924_01.jpg[Russell in 1924 01] by Unknown - http://www.humanities.mcmaster.ca/~bertrand/later.html. Licensed under Public domain via http://commons.wikimedia.org/wiki/[Wikimedia Commons].
...
statements to make complex statements. So two simple statements might be:

[example]
Abelard likes coffee. +
Abelard likes cake.

And a complex statement formed from these two simple statements is:

[example]
Abelard likes coffee and Abelard likes cake.

This complex statement is true if both the simple statements are true, otherwise
it is false. Another complex statement we can make from our two simple
statements is:

[example]
Abelard likes coffee or Abelard likes cake.

This complex statment is false if both simple statements are false, otherwise
it's true. Those mathematicians from the Ivory Tower use the term _connectives_
for the '`and`' and '`or`' that join simple statements to make complex
statements.

:numbered!:
==== Questions

. Are the following complex statements true or false?
...
it can be any capital letter of the alphabet. So for the complex statement:

[example]
Abelard likes coffee and Abelard likes cake.

the two simple statements can have the atomic formulae P and Q:
...
and write it using the atomic formulae to give the formula:
...
. Write the following compound statements as formulae:
.. The film Erin Brokovich stars Julia Roberts and 16 is greater than 4.
.. London is the capital of France or Paris is the capital of France.
.. Some people have brown eyes and humans lay eggs.
.. Four multiplied by two is twenty or it has never rained in Wales.
...
Using formulae does genuinely make it a lot easier to write down statements, I
...
. Write the answers to the questions for <<_formulae>> in logical symbols.
...
Rather fussy. But I found that it makes sense later on when formulae become
more complicated. 
...
. Write the answers to the questions for <<_formulae>> using logical symbols.
...
(T) or false (F). For the two atomic formulae P and Q, the truth table for
(P {and} Q) is:
...
There's another connective called _negation_, which has the symbol {not} and in
English sentences appears as the word '`not`'. Its truth table is:
...
. Say whether each of the following formulae is a tautology, a contradiction
  or both:
...
. Write the following English statements as logical formulae:
...
. Create a truth table for each of the following formulae:
.. ({not}P {implies} Q)
.. (Q {implies} {not}Q)
.. ((P {implies} Q) {or} P)
...
Where the comma-separated list of formulae on the left of the double-turnstile
...
exponentially with the number of propositions. The logicians have discovered /
...
| Double negation        | {not}{not}A {entails} A
...
_Equivalence_ is a binary connective with the mystical symbol {equiv} and its
truth table is:
...
If two formulas are equivalent, then the formula formed by joining them with the
{equiv} connective will be a tautology. For example, if the two formulas:
...
will be a tautology. Let's check by writing its truth table:

|===
| A | {not}A | {not}{not}A | (A {equiv} {not}{not}A)

| T | F      | T           | T
| F | T      | F           | T
|===

it's a tautology, so the two formulae are equivalent.
...
. Give the truth table for each of the following formulae:
.. (A {equiv} (B {and} C))
.. (B {or} (A {equiv} B))
.. (P {and} {not}(P {equiv} (Q {or} P)))

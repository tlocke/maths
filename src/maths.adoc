...
. For all the arguments in the questions for <<_arguments>>, show that they are
  valid by using rules of inference. (Warning: I can't do the last three, so
  there aren't any answers for them yet. Let me know if you have the answer!)
...
connective with the symbol {equivalent}. It's precedence is after {implies} and
it's truth table is:
...
.. P: Ben won a tennis match. +
   Q: Toby won a tennis match. +
   P {or} Q, {not}(P {and} Q), Q {entails} {not}P
   1. I'm afraid I can't do this one. Let me know if you work out the answer!
.. P: Bill orders 6x. +
   Q: Bill orders Tribute. +
   R: The pub is open. +
   P {or} Q, P {or} Q {implies} R, {not}Q {entails} R {and} P

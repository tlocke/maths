...
is a tautology, as shown by truth table:
...
is not a tautology, as shown by truth table:
...
.. If: +
   ...
   is a tautology. The truth table is:
+
[cols="8*", options="header"]
|===
| A
...
|===
+
which shows it is a tautology, and so the two formulas are equivalent.
.. If: +
   (A {or} B) {equiv} {not}({not}A {and} {not}B) +
   then: +
   (A {or} B) {bicond} {not}({not}A {and} {not}B) +
   is a tautology. The truth table for this formula is:
+
[cols="8*", options="header"]
|===
| A
...
|===
+
which shows it is a tautology, and so the two formulas we started with are
equivalent.
...
.. If the argument P {implies} Q, P {entails} Q is valid, then the formula
   \((P {implies} Q) {and} P {implies} Q) is a tautology. The following truth
   table shows the formula is a tautology, so the argument is indeed valid.
+
|===
| 1 | 2 | 3             | 4             | 5
...
.. If the argument A {or} B, {not} B {entails} A is valid, then the formula
   \(\((A {or} B) {and} {not}B) {implies} A) is a tautology. The following truth
   table shows the formula is a tautology, so the argument is indeed valid.
+
|===
| 1 | 2 | 3        | 4      | 5                 | 6
...
.. If the argument P {or} Q, Q {entails} {not}P is valid, then the formula
   \(\((P {or} Q) {and} Q) {implies} {not}P) is a tautology. The following truth
   table shows the formula is not a tautology, so the argument is not valid.
+
|===
| 1 | 2 | 3        | 4      | 5             | 6
...
.. If the argument P {or} Q, P {or} Q {implies} R, {not}Q {entails} R {and} P is
   ...
   {and} {not}Q) {implies} (R {and} P)) is a tautology. The following truth
   table shows the formula is a tautology, so the argument is valid.
+
...
.. If the argument P {or} Q, {not}(P {and} Q), {not}P {entails} Q is valid, then
   ...
   Q) is a tautology. The following truth table shows the formula is a
...
quadratically with the number of atomic formulas. The logicians have discovered
...
which shows that the formula is a tautology and so the argument is valid.
...
* http://logic.stanford.edu/classes/cs157/2010/notes/chap02.html (Excellent
  notes, but sadly no longer available).

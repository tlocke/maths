...
There are a couple more connectives (with their own mystical symbols!) that
logicians use, _equivalence_ and _exclusive-or_. Equivalence is a binary
connective with the symbol {equiv}. It's precedence is after {implies} and
it's truth table is:
...
If two expressions are equivalent, then the expression formed by connecting
them with the {equiv} connective will be a tautology. For example, if the two
expressions:
...
will be a tautology. Let's find out by writing its truth table:
...
it's a tautology, so the two expressions are equivalent.
...
. Give the truth table for each of the following expressions:
.. A {equiv} B {and} C
.. B {or} (A {equiv} B)
